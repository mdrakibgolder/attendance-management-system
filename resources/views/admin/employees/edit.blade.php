    ...
    <div class="py-8">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="rounded-xl bg-white shadow-sm p-6">
                <form method="POST" action="{{ route('admin.employees.update', $employee) }}" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @csrf
                    @method('PATCH')
                    @include('admin.employees.partials.form')
                    <div class="md:col-span-2 flex gap-3">
                        <button class="rounded-md bg-indigo-600 px-4 py-2 text-white hover:bg-indigo-700">Update</button>
